"""
Example: Async Function Execution with Result Polling (Async/Await)

Demonstrates how to:
1. Execute a function asynchronously
2. Get the task_id from the invocation
3. Poll for the result using get_result() with async/await
4. Multiple authentication methods in async mode
"""
...
async def main():
    ...
    result = await auth_client.functions.execute(
        "process-order",
    ...
    while attempt < max_attempts:
        ...
        # Get the task result
        task_result = await auth_client.functions.get_result(task_id)
